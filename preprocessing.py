...
OUT_DIR = "./datasets_tests/MIRFlickr/"
...
final_tags = {
    idx: tags
    for idx, tags in filtered_tags.items()
    if idx in valid_ids
}
...
pairs = sorted(
    pairs,
    key=lambda x: int(
        ''.join(filter(str.isdigit, os.path.basename(x[0])))
    )
)
...
def extract_features(pair_list):
    ...
    return np.array(I), np.array(Tvec), np.array(L), np.array(IDs)
